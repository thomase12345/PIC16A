...
#Spotipy setup to access user's music data
@app.route('/')
def login():
...
#Home page for website, shows user's most played tracks recently
@app.route('/home')
def home():
...
def genre_recommendations():
    #Look up the user’s top genres and then get five tracks for each of those top genres.
    token_info = session.get('token_info', {})
    if not token_info:
        return redirect('/')
...
def recommend_similar_with_genres(song_name, df, features_scaled, nn_model, sp, top_n=10):
    ...
    for idx in indices[0][1:top_n+1]:  #skip the input song so it's not included in recommendations
        track = df.iloc[idx]
        try:
